...
import { submitSchema, type SubmitFormValues } from '@repo/common/types'
...
const AddSubmitForm = (props: any) => {
  ...
  async function onsubmit(data: any) {
    ...
    setOnLoading(true)
    const newUrl = `${url}/api/v1/submit/${props.id}`
  ...
  return (<div>
    <button onClick={Add} className='flex gap-1 border lg:px-2 px-1 lg:py-1 cursor-pointer lg:rounded-lg rounded-sm lg:font-medium font-normal bg-gray-100 hover:bg-gray-300 text-black items-center lg:text-base text-xs'><div className=' text-black'><Image src={Plus} alt='Plus' className='lg:w-4 w-3'></Image></div>Add Submission</button>
      ...
      <form onSubmit={handleSubmit(onsubmit)}>
        <div className='w-screen h-screen bg-slate-700/70 fixed top-0 left-0  flex justify-center items-center backdrop-blur-sm  z-20 text-black'>
          <div className='flex flex-col p-8  gap-2 bg-white rounded-2xl relative'>
            ...
            <div className='text-3xl p-1 text-center'>Add Submission</div>
