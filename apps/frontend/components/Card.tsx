...
export const Card = (props: any) => {
  ...
  return (<div>
    <div className='flex border p-4 min-w-[20vw] gap-4 justify-between rounded-2xl'>
      <div className='text-white  flex flex-col gap-1'>

        {(props.Profile || props.personalProfile) ? <div>Name of Project:&nbsp;<Link className='bg-gray-900 hover:bg-gray-600 px-2 rounded-md' href={`/project/${props.project.id}`}>{props.project.name}</Link></div> :
          <div>Name:&nbsp;<Link className='bg-gray-900 hover:bg-gray-600 px-2 rounded-md' href={`/profile/${props.devId}`}>{props.devName}</Link></div>
        }
